function buildMessage(bucketId, netEdgeAfterFriction, blockedReason) {
    if (blockedReason)
        return `类别 ${bucketId} 当前阻断：${blockedReason}`;
    return `类别 ${bucketId} 当前净优势 ${netEdgeAfterFriction.toFixed(4)}，可进入人工执行队列。`;
}
export function buildExecutionPlan(input, verdict) {
    return {
